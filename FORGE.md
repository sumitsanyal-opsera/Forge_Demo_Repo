...
## WO-124: User Story: WO-124 - Dockerfiles and Docker Compose for Local Dev
- **Status:** completed
- **Commit:** `4153fad`
- **Files:** 11 (+498/-6)
- **Duration:** 373ss
- **Approach:** WO-124 creates production-grade Dockerfiles for the four core services and extends the existing docker-compose.yml. Directory mapping: WO says services/detection/ but the repo uses services/detection-service/ — all files placed in the actual directories. The WO-074 root Dockerfile targets the api-gateway; this WO creates per-service Dockerfiles for the 4 core services (detection, baseline, alert, dashboard). All four Dockerfiles follow the same 3-stage pattern: (1) deps stage installs npm packages with --ignore-scripts and explicit prisma generate; (2) builder stage copies source, compiles shared then service TypeScript, prunes devDependencies; (3) production stage uses gcr.io/distroless/nodejs20-debian12 with USER 1000 and a node-based HEALTHCHECK. The existing docker-compose.yml (postgres + redis from WO-090) was extended with the 4 application services, shared network, resource limits, and service_healthy depends_on conditions. The /api/v1/health/live and /api/v1/health/ready probe routes were added to all 4 service src/index.ts files. Prisma schema updated with binaryTargets for distroless Debian 12 runtime. .env.example extended with Docker/Prisma guidance.

## WO-091: User Story: WO-091 - Create Complete Database Schema with Partitioning
- **Status:** completed
- **Commit:** `86eb273`
- **Files:** 11 (+2305/-4)
- **Duration:** 946ss
- **Approach:** Implemented the complete database schema using a hybrid Prisma + raw SQL strategy. schema.prisma defines 8 enums and 11 models with all relationships; the hand-authored migration SQL handles partitioned DDL (PARTITION BY RANGE on started_at), composite PKs (id+startedAt) required by PostgreSQL partitioning, composite FKs for tables referencing partitioned tables, GIN indexes on all JSONB columns using jsonb_path_ops, a partial unique index for ThresholdConfig org-level defaults (NULL pipeline_id), monthly partitions for 2026-06 through 2026-11, and the audit_log immutability trigger. Seed data creates 2 users, 2 Salesforce orgs, 3 monitored pipelines, 54 executions+steps across 3 months, threshold configs, baselines, detection events, alert records, alert recipients, and audit log entries. Three integration test files cover all tables including the audit trigger behaviour.

...
## WO-084: User Story: WO-084 - CLI Entry Point to Run Validation and Print Results
- **Status:** completed
- **Commit:** `040e629`
- **Files:** 3 (+119/-13)
- **Duration:** 344ss
- **Approach:** WO-084 required a validate(xmlPath, schemaPath) export, VALID/INVALID console output, and integration tests. src/index.ts already existed from WO-090/094. The key changes: (1) Added export function validate() that calls loadSchema() from src/validate.ts to get a libxmljs2 Document, then passes it to xml-validator.validateXml() (WO-083) — this returns {isValid, errors: string[]} matching the AC shape. (2) Added formatValidation() helper for VALID/INVALID output format. (3) Updated the WO-090 single-arg positional fallback: when only xmlPath is given (no xsdPath), now calls validate() with BUNDLED_SCHEMA_PATH (schemas/CustomObject.xsd) instead of well-formedness-only parseXml(). When xsdPath is also given, keeps the old validateAgainstXsd/formatSingleResult path intact. (4) Updated usage message to 'Usage: validate <xml-file-path>'. (5) Added 2 unit tests for validate() to the existing src/__tests__/index.test.ts using test-data/ fixtures (which were created for schemas/CustomObject.xsd in WO-093) — test-fixtures/valid-CustomObject.xml is NOT used because it doesn't conform to the bundled schema (has label/pluralLabel/deploymentStatus, missing fullName). (6) Created src/__tests__/integration/cli.test.ts with 4 execSync tests.

## WO-075: User Story: WO-075 - Create CLI Runner and Console Results Output
- **Status:** completed
- **Commit:** `d307026`
- **Files:** 2 (+67/-7)
- **Duration:** 500ss
